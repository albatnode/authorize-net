...
function createJsonCallback (cb) {
  ...
  return function (res, body) {
    console.log('[createJsonCallback body]', body);
    return toJson(body)
      .then(function (result) {
        ...
        return cb(json);
      }).catch(err => console.log('[createJsonCallback error]', err));
  }
}
...
AuthorizeNetGateway.prototype.getHeldTransactionListRequest = function getHeldTransactionListRequest () {

  var service = this;

  return P.resolve()
    .then(function () {
      return setRequest(service, 'getUnsettledTransactionListRequest', {
        key: 'status',
    ...
    .then(sendXmlifiedRequest(service))
    .spread(createJsonCallback(function (json) {
      if (json.getUnsettledTransactionListResponse) {
        console.log('[json]', util.inspect(json, {showHidden: false, depth: null}));
        var mainErrMessage  = _.get(json, ['getUnsettledTransactionListResponse', 'messages', 0]);
        var mainErrMessageResultCode  = _.get(mainErrMessage, ['resultCode', 0]);
        var mainErrMessageCode  = _.get(mainErrMessage, ['message', 0, 'code']);
        var mainErrMessageText  = _.get(mainErrMessage, ['message', 0, 'text']);
        var mainErrMessageCommonText = ((mainErrMessageCode || mainErrMessageText) && mainErrMessageResultCode === 'Error') ?
        ...
        var transactionResponseErr = _.get(json,
          ['getUnsettledTransactionListResponse', 'errors', 0, 'error', 0]);
        var transactionResponseErrCode = _.get(transactionResponseErr, ['errorCode']);
        var transactionResponseErrText = _.get(transactionResponseErr, ['errorText']);
        var transactionResponseErrCommonText = (transactionResponseErrCode || transactionResponseErrText) ?
          'code: ' + transactionResponseErrCode + ', text: ' + transactionResponseErrText : null;
        ...
        if (Array.isArray(commonErrorText) && commonErrorText.length > 0) {
          throw new GatewayError(commonErrorText.join(', '), json.getUnsettledTransactionListResponse);
        }
        const transactionPath = ['getUnsettledTransactionListResponse', 'transactions', 0, 'transaction'];
        return _.get(json,transactionPath);

      } else {
        throw new Error('Can not parse unsettled transaction answer from gateway');
      }
    }))
};
