...
var customerSchema = schemas.customer;
/**
 *
 * @param options
 * @constructor
  ...
  return function (request) {
    return P.resolve()
      .then(function () {
        ...
        // console.log('[sendXmlifiedRequest request]', util.inspect(request, {showHidden: false, depth: null}));
        var xmlContent = builder.buildObject(request);
        // console.log('[sendXmlifiedRequest xmlContent]', util.inspect(xmlContent, {showHidden: false, depth: null}));
  ...
  return P.resolve()
    ...
    .spread(createJsonCallback(function (json) {
      if (json.createTransactionResponse) {
        // console.log('[json]', util.inspect(json, {showHidden: false, depth: null}));
        var mainErrMessage  = _.get(json, ['createTransactionResponse', 'messages', 0]);
        var mainErrMessageResultCode  = _.get(mainErrMessage, ['resultCode', 0]);
        var mainErrMessageCode  = _.get(mainErrMessage, ['message', 0, 'code']);
        var mainErrMessageText  = _.get(mainErrMessage, ['message', 0, 'text']);
        var mainErrMessageCommonText = ((mainErrMessageCode || mainErrMessageText) && mainErrMessageResultCode === 'Error') ?
          'code: ' + mainErrMessageCode + ', text: ' + mainErrMessageText : null;

        var transactionResponseErr = _.get(json,
          ['createTransactionResponse','transactionResponse',0,'errors', 0, 'error', 0]);
        var transactionResponseErrCode = _.get(transactionResponseErr, ['errorCode']);
        var transactionResponseErrText = _.get(transactionResponseErr, ['errorText']);
        var transactionResponseErrCommonText = (transactionResponseErrCode || transactionResponseErrText) ?
          'code: ' + transactionResponseErrCode + ', text: ' + transactionResponseErrText : null;
        var commonErrorText = [];
        if (mainErrMessageCommonText) {
          commonErrorText.push(mainErrMessageCommonText);
        }
        if (transactionResponseErrCommonText) {
          commonErrorText.push(transactionResponseErrCommonText);
        }
        // console.log('[mainErrMessage]', mainErrMessage);
        // console.log('[mainErrMessageResultCode]', mainErrMessageResultCode);
        // console.log('[mainErrMessageCode]', mainErrMessageCode);
        // console.log('[mainErrMessageText]', mainErrMessageText);
        // console.log('[mainErrMessageCommonText]', mainErrMessageCommonText);
        // console.log('[transactionResponseErr]', transactionResponseErr);
        // console.log('[transactionResponseErrCode]', transactionResponseErrCode);
        // console.log('[transactionResponseErrText]', transactionResponseErrText);
        // console.log('[transactionResponseErrCommonText]', transactionResponseErrCommonText);
        if (Array.isArray(commonErrorText) && commonErrorText.length > 0) {
          throw new GatewayError(commonErrorText.join(', '), json.createTransactionResponse);
        }
...
AuthorizeNetGateway.prototype.submitTransaction = function submitTransaction (order, creditCard, prospect, other) {
  ...
  // console.log('[prospect]', util.inspect(prospect, {showHidden: false, depth: null}));
  if (prospect) {
    body.customer = mapKeys(prospect, customerSchema);
    body.billTo = mapKeys(prospect, billToSchema);
    body.shipTo = mapKeys(prospect, shipToSchema);
  }
  // console.log('[payment body]', util.inspect(body, {showHidden: false, depth: null}));
