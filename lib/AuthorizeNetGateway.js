  ...
  return P.resolve()
    ...
    .spread(createJsonCallback(function (json) {
      if (json.createTransactionResponse) {
        ...
        var mainErrMessageResultCode  = _.get(mainErrMessage, ['resultCode']);
        var mainErrMessageCode  = _.get(mainErrMessage, ['message', 0, 'code']);
        var mainErrMessageText  = _.get(mainErrMessage, ['message', 0, 'text']);
        var mainErrMessageCommonText = 'code: ' + mainErrMessageCode + ', text: ' + mainErrMessageText;
        var transactionResponseErr = _.get(json,
          ['createTransactionResponse','transactionResponse',0,'errors',0]);
        ...
        var transactionResponseErrCommonText = 'code: ' + transactionResponseErrCode + ', text: ' + transactionResponseErrText;
        if (mainErrMessageResultCode === 'Error') {
          throw new GatewayError(mainErrMessageCommonText, json.createTransactionResponse);
        }  else if(transactionResponseErr){
          throw new GatewayError(transactionResponseErrCommonText, json.createTransactionResponse);
        }
